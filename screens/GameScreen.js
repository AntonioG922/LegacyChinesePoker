...
import {
  ...
  dealCards,
  findStartingPlayer,
  GAME_TYPE_BY_NUMBER_OF_PLAYERS,
  GAME_TYPES,
  AI_UID_PREFIXES,
  AVATARS,
...
export default function GameScreen({ route, navigation }) {
  ...
  const gameLobby =
    gameData.gameName.length > 20 ? "PlayNowGames" : "CustomGames";
  ...
  useEffect(() => {
    ...
    if (!isLocalGame) {
      unsubscribe = db
        ...
        .doc(gameData.gameName)
        .onSnapshot((doc) => {
          const docData = doc.data();
          const leavingGame =
            docData === undefined ||
            (!Object.keys(docData.players).includes(user.uid) &&
              !Object.keys(docData.rejoinablePlayers).includes(user.uid));

          const currentPlayerIsRejoinable = Object.keys(
            docData.rejoinablePlayers
          ).includes(user.uid);
  ...
  useEffect(() => {
    const isCurrentPlayer =
      gameData.players[user.uid] === gameData.currentPlayerTurnIndex;
  ...
  useEffect(() => {
    if (
      ...
      Object.keys(gameData.playersPlayingAgain).length === 0
    ) {
      setGameStarted(true);
  ...
  useEffect(() => {
    if (
      gameData.numberOfPlayers === gameData.places.length &&
      Object.keys(gameData.playersPlayingAgain).length === 0
    ) {
      if (!isLocalGame) {
        updateUserStats();
  ...
  useEffect(() => {
    const playersPlayingAgainLength = Object.keys(gameData.playersPlayingAgain)
      .length;
    const lastUIDPlayingAgain = playersPlayingAgainLength
      ? Object.keys(gameData.playersPlayingAgain)[playersPlayingAgainLength - 1]
      : null;
    const allRemainingPlayersPlayingAgain =
      playersPlayingAgainLength === Object.keys(gameData.players).length;

    if (allRemainingPlayersPlayingAgain && lastUIDPlayingAgain === user.uid) {
      const hands = dealCards(
        gameData.useJoker,
        gameData.numberOfPlayers,
        gameData.cardsPerPlayer
      );
      let players = {};
      ...
      const updates = {
        ...
        queue: queue,
        isLocalGame:
          gameData.numberOfPlayers === gameData.numberOfComputers + 1,
        rejoinablePlayers: {},
        rejoinedPlayers: {},
      };
      ...
      if (isLocalGame) {
        ...
        db.collection(gameLobby)
          ...
          .catch(() => {
            alert(
              "Error trying to play again. Please check your connection and try again."
            );
          });
      }
    }
  }, [
    Object.keys(gameData.playersPlayingAgain).length,
  ...
  useEffect(() => {
    ...
    if (isLocalGame && !lastPlayerLeft) {
      const currentPlayerUID = Object.keys(gameData.players).find(
        (uid) => gameData.players[uid] === gameData.currentPlayerTurnIndex
      );
      const computerPlaysNext = AI_UID_PREFIXES.includes(
        currentPlayerUID.slice(0, 7)
      );

      if (computerPlaysNext) {
        const AIDifficulty = currentPlayerUID.slice(3, 7);
        const exclusive = AIDifficulty === "Easy" ? false : true;
        const numPlayersStillIn =
          gameData.numberOfPlayers - gameData.places.length;
        const botCards =
          gameData.hands[gameData.players[currentPlayerUID]].cards;
        const computerSelectedCards = getLowestPlayableCards(
          botCards,
          gameData.cardsPerPlayer,
          numPlayersStillIn,
          gameData.currentHandType,
          gameData.lastPlayed,
          exclusive
        );
        setTimeout(
  ...
  function updateUserStats() {
    ...
    let allGamesUpdates = {
      ...
      playtime: firebase.firestore.FieldValue.increment(
        Date.now() - gameData.gameStartTime
      ),
      placements: {
        1:
          userPlacement === 1
        ...
        last:
          userPlacement === gameData.numberOfPlayers
            ? firebase.firestore.FieldValue.increment(1)
            : firebase.firestore.FieldValue.increment(0),
      },
    };
    ...
    if (Object.keys(handsPlayed).length > 0) {
      ...
      Object.keys(handsPlayed).forEach((handType) => {
        hands[handType] = firebase.firestore.FieldValue.increment(
  ...
  function maybeSetGameStartTime() {
    const isLastPlayer =
      Object.keys(gameData.players).find(
        (key) => gameData.players[key] === gameData.numberOfPlayers - 1
      ) === user.uid;
  ...
  function everyonePassAfterWinner() {
    ...
    if (gameData.places.length === 0) return false;

    const lastUIDToPlay = Object.keys(gameData.lastPlayerToPlay)[0];
    const lastUIDToPlayTurnIndex = gameData.players[lastUIDToPlay];
    if (
      lastUIDToPlayTurnIndex &&
      gameData.hands[lastUIDToPlayTurnIndex].cards.length
    )
      return false;

    const lastPlayTurnNum = Number(
      Object.keys(gameData.playersTurnHistory[lastUIDToPlay]).pop()
    );
    let index = 0;
    for (let hand of gameData.hands) {
      if (hand.cards.length) {
        const remainingPlayerUID = Object.keys(gameData.players).find(
          (key) => gameData.players[key] === index
        );
        const remainingPlayerLastPlayTurnNum = Number(
          Object.keys(gameData.playersTurnHistory[remainingPlayerUID]).pop()
        );
        const remainingPlayerLastPlay =
          gameData.playersTurnHistory[remainingPlayerUID][
            remainingPlayerLastPlayTurnNum
          ];
        const currentPlayerUID = Object.keys(gameData.players).find(
          (uid) => gameData.players[uid] === gameData.currentPlayerTurnIndex
        );
        ...
        if (
          (remainingPlayerLastPlayTurnNum < lastPlayTurnNum ||
            (remainingPlayerLastPlayTurnNum > lastPlayTurnNum &&
              remainingPlayerLastPlay !== "PASS")) &&
  ...
  function playCards(selectedCards) {
    ...
    const everyonePassed =
      gameData.currentHandType === HAND_TYPES.START_OF_ROUND;
    const isFirstPlayOfGame =
      gameData.currentHandType === HAND_TYPES.START_OF_GAME;
    const currentHand =
      gameData.hands[gameData.players[currentPlayerUID]].cards;
    ...
    if (playedHandType === HAND_TYPES.INVALID) {
      ...
      isFirstPlayOfGame &&
      !selectedCards.includes(getLowestCard(currentHand))
    ) {
      ...
      !everyonePassed &&
      !isLegalPlay(playedHandType, gameData.currentHandType)
    ) {
    ...
    hands[player].cards = hands[player].cards.filter(
    ...
    if (handIsEmpty) {
      places = isLocalGame
        ? [...places, currentPlayerUID]
        : firebase.firestore.FieldValue.arrayUnion(user.uid);

      if (!gameData.places.length) {
        gamesWon[currentPlayerUID] += 1;
        gamesPlayed = isLocalGame
          ? gameData.gamesPlayed + 1
          : firebase.firestore.FieldValue.increment(1);
      }

      if (gameData.places.length === gameData.numberOfPlayers - 2) {
        const lastPlaceUID = Object.keys(gameData.players).find(
          (playerUid) =>
            playerUid !== currentPlayerUID &&
            !gameData.places.includes(playerUid)
        );
        places = isLocalGame
          ? [...gameData.places, currentPlayerUID, lastPlaceUID]
          : firebase.firestore.FieldValue.arrayUnion(user.uid, lastPlaceUID);
      }
    }

    const data = {
      playedCards: isLocalGame
        ? [...gameData.playedCards, ...selectedCards]
        : firebase.firestore.FieldValue.arrayUnion(...selectedCards),
      lastPlayed: selectedCards,
      lastPlayerToPlay: {
        [currentPlayerUID]: gameData.displayNames[currentPlayerUID],
      },
      hands: hands,
    ...
    updatedHandsPlayed[playedHandType] =
      (handsPlayed[playedHandType] || 0) + selectedCards.length;
  ...
  function pass() {
    ...
    const nextPlayerStillInUID = Object.keys(gameData.players).find(
      (uid) => gameData.players[uid] === getNextNonEmptyHandIndexLocal()
    );
    const nextPlayerPlayedLast =
      nextPlayerStillInUID === Object.keys(gameData.lastPlayerToPlay)[0];
    const nextPlayerHadBotPlayForThemLast =
      gameData.rejoinedPlayers[nextPlayerStillInUID] ===
      Object.keys(gameData.lastPlayerToPlay)[0];
    const everyonePassed =
      nextPlayerPlayedLast ||
      nextPlayerHadBotPlayForThemLast ||
      everyonePassAfterWinner();
    ...
    const update = {
      currentHandType: everyonePassed
        ? HAND_TYPES.START_OF_ROUND
        : gameData.currentHandType,
      currentPlayerTurnIndex: getNextNonEmptyHandIndexLocal(),
      playersTurnHistory: playersTurnHistory,
      overallTurnHistory: overallTurnHistory,
  ...
  function playAgain() {
    ...
    if (isLocalGame) {
      ...
      let usedBotUIDs = [];
      while (tempNumComps--) {
        const botUID = Object.keys(gameData.players).find(
          (uid) =>
            AI_UID_PREFIXES.includes(uid.slice(0, 7)) &&
            !usedBotUIDs.includes(uid)
        );
    ...
    if (isLocalGame) {
      ...
      db.collection(gameLobby)
        ...
        .catch(() => {
          alert(
            "Error trying to play again. Please check your internet connection and try again."
          );
        });
    }
  }
    ...
    if (isLocalGame) {
      ...
      if (Object.keys(gameData.players).length === 1) {
        db.collection(gameLobby)
          ...
          .catch((error) => {
            alert(
              "Error exiting game. Please check your connection and try again."
            );
  ...
  function getDisplayNameRotation(index) {
    if (
      ...
      (gameData.numberOfPlayers === 4 && index == 1)
    ) {
      return "180deg";
    }
  ...
  function Menu() {
    const [currentAvatar, setCurrentAvatar] = useState(
      gameStarted ? gameData.hands[gameData.players[user.uid]].avatar : null
    );
      ...
      if (isLocalGame) {
        ...
        db.collection(gameLobby)
          ...
          .catch((error) => {
            setLoading(false);
            alert(
              "Error changing avatar. Please check your internet connection and try again."
            );
    ...
    function isTakenAvatar(avatar) {
      return gameData.hands.some(
        (hand, index) =>
          hand.avatar === avatar && index !== gameData.players[user.uid]
      );
    }
        ...
        {typeof gameData.players[user.uid] === "number" && (
          <View style={{ flex: 1 }}>
            <View style={styles.mainContent}>
              <View>
                <HeaderText style={{ alignSelf: "center", fontSize: 30 }}>
                  {gameData.gameName}
                </HeaderText>
              ...
              <View>
                <HeaderText style={[styles.text, { alignSelf: "center" }]}>
                  Favorite Icon
                </HeaderText>
                    ...
                    return (
                      ...
                      >
                        <Image
                          ...
                          style={[
                            styles.avatarImage,
                            currentAvatar === AVATARS[avatar]
                              ? styles.currentAvatar
                              : null,
                            isTakenAvatar(avatar) ? styles.takenAvatar : null,
                          ]}
                        />
                      </TouchableOpacity>
                    );
              ...
              <View>
                ...
                <TouchableOpacity
                  style={styles.row}
                  onPress={() => setShowDisplayNames(!showDisplayNames)}
                >
                  <HeaderText style={[styles.text]}>Display Names</HeaderText>
                  <HeaderText style={styles.iconContainer}>
                    <FontAwesome5
                      name={showDisplayNames ? "check" : "times"}
                      style={[
                        styles.text,
                        {
                          color: showDisplayNames
                            ? "rgb(80, 189, 68)"
                            : "rgb(217, 56, 27)",
                        },
                      ]}
    ...
    return (
      <Animated.View
        style={{
          ...
          opacity: screenShaderOpacity,
          zIndex: screenShaderZindex,
        }}
      >
        <TouchableHighlight
  ...
  function RejoinScreen() {
    ...
    function rejoinGame() {
      ...
      updates[`playersTurnHistory.${user.uid}`] =
        gameData.playersTurnHistory[botUID];
      updates[
        `rejoinablePlayers.${user.uid}`
      ] = firebase.firestore.FieldValue.delete();
      updates[`rejoinedPlayers.${user.uid}`] =
        gameData.rejoinablePlayers[user.uid];
      ...
      if (gameData.places.includes(botUID)) {
        tempPlaces[gameData.places.findIndex((uid) => uid === botUID)] =
          user.uid;
      ...
      firebase
        ...
        .catch((error) => {
          setLoading(false);
          alert(
            "Error rejoining game. Please check your internet connection and try again."
          );
    ...
    return (
      ...
      >
        <Loader
          ...
          style={{ width: loading ? "100%" : 0, height: loading ? "100%" : 0 }}
        />
        <View
          style={{
            height: "100%",
    ...
    >
      {!showRejoinScreen && (
        <View style={{ flex: 1 }}>
              ...
              leavingGame
                ...
                : `Waiting for ${
                    (Object.keys(gameData.playersPlayingAgain).length
                      ? gameData.numberOfPlayers -
                        Object.keys(gameData.playersPlayingAgain).length
                      : false) || gameData.playersLeftToJoin
                  } more player${
                    (
                      Object.keys(gameData.playersPlayingAgain).length
                        ? gameData.numberOfPlayers -
          ...
          <PopUpMessage
            showPopUp={showPopUp}
            exitAction={leavingGame ? () => {} : dontPlayAgain}
            exitMessage="No"
            confirmAction={playAgain}
            confirmMessage="Yes"
          >
            {gameData.places.map((player, index) => {
              ...
              const gamesWon =
                gameData.gamesPlayed > 1 ? gameData.gamesWon[player] : null;
              return (
                <TrophyPlaceDisplay
                  key={index}
                  place={index}
            ...
            {!(gameData.numberOfComputers === gameData.numberOfPlayers - 1) && (
              <PlayAgainTimer dontPlayAgain={dontPlayAgain} />
            )}
          </PopUpMessage>
          ...
          <PlayedCardsContainer
            ...
            lastPlayerToPlay={
              gameData.lastPlayerToPlay[
                Object.keys(gameData.lastPlayerToPlay)[0]
              ]
            }
            avatarImage={getAvatarImage(
              gameData.hands[gameData.currentPlayerTurnIndex].avatar
            )}
          ...
          {gameStarted && (
            <View style={styles.container}>
              <UserCardContainer
                cards={gameData.hands[gameData.players[user.uid]].cards}
                ...
                avatarImage={getAvatarImage(
                  gameData.hands[gameData.players[user.uid]].avatar
                )}
                playCards={playCards}
                pass={pass}
                style={styles.player1Hand}
              />
              {Array.from({ length: gameData.numberOfPlayers - 1 }).map(
                (value, index) => {
                  const playerIndex =
                    (gameData.players[user.uid] + index + 1) %
                    gameData.numberOfPlayers;
                  const displayName = showDisplayNames
                    ? gameData.displayNames[
                        Object.keys(gameData.players).find(
                          (key) => gameData.players[key] === playerIndex
                        )
                      ]
                    : "";
                  ...
                  return (
                    <FaceDownCardsContainer
                      key={playerIndex}
                      numberOfCards={gameData.hands[playerIndex].cards.length}
                      style={[styles.opposingPlayerHand, getStyle(index + 2)]}
                      displayName={displayName}
                      displayNameStyling={{
                        transform: [{ rotateZ: getDisplayNameRotation(index) }],
                      }}
                      avatarImage={getAvatarImage(
                        gameData.hands[playerIndex].avatar
                      )}
                      avatarStyling={{
                        transform: [{ rotateZ: getAvatarRotation(index) }],
                      }}
                      isCurrentPlayer={
                        playerIndex === gameData.currentPlayerTurnIndex
                      }
                    />
                  );
              ...
              >
                <Animated.View
                  style={[
                    ...
                    {
                      opacity: hamButtonOpacity,
                      width: hamButtonWidth,
                      transform: [{ translateY: hamButtonY1 }],
                    },
                  ]}
                />
                <Animated.View
                  style={[
                    styles.hamburgerButton,
                    { transform: [{ rotateZ: hamButtonRotationDeg1 }] },
                  ]}
                />
                <Animated.View
                  style={[
                    styles.hamburgerButton,
                    {
                      transform: [{ rotateZ: hamButtonRotationDeg2 }],
                      position: "absolute",
                    },
                  ]}
                />
                <Animated.View
                  style={[
                    styles.hamburgerButton,
                    {
                      opacity: hamButtonOpacity,
                      width: hamButtonWidth,
                      transform: [{ translateY: hamButtonY2 }],
                    },
                  ]}
                />
              </View>
...
function PlayAgainTimer({ dontPlayAgain }) {
  ...
  return <HeaderText style={{ fontSize: 20 }}>{countdownNum}</HeaderText>;
}
...
const styles = StyleSheet.create({
  ...
  playedCards: {
    ...
    left: "50%",
    top: "50%",
    transform: [{ translateX: -40 }, { translateY: -53.75 }],
    alignItems: "center",
  },
  player1Hand: {
    bottom: 40,
